(()=>{
  ...
  if(!window.CMDPersistentSite){const s=document.createElement('script');s.src='/persistent-site-browser.js?v=20260824-4';document.head.appendChild(s)}
  ...
  audio.addEventListener('play',()=>{reveal();play.textContent='❚❚';play.setAttribute('aria-label','Pause');status.textContent='Playing';setTriggerState();window.CMDPersistentSite?.setSession(true);window.CMDPersistentSite?.refreshClearance?.();if('mediaSession'in navigator){try{navigator.mediaSession.playbackState='playing'}catch{}}});
