(()=>{
  ...
  const setTriggerState=()=>{
    ...
    triggers.forEach(button=>{
      const base=button.dataset.baseLabel||button.textContent.replace(/^▶\s*/,'').replace(/^❚❚\s*/,'');
      button.dataset.baseLabel=base;
      button.textContent=`${playing?'❚❚':'▶'} ${base}`;
    });
  };

  const reveal=()=>{
    dock.hidden=false;
    document.body.classList.add('pick-player-open');
  };

  const toggle=()=>{
    reveal();
    if(audio.paused)audio.play().catch(()=>{status.textContent='Tap play to start';});else audio.pause();
  };
  ...
  audio.addEventListener('ended',()=>{status.textContent='Finished';play.textContent='▶';bar.style.width='100%';setTriggerState();});
  audio.addEventListener('error',()=>{status.textContent='This audio file could not be loaded.';play.textContent='▶';setTriggerState();});
  ...
  if('mediaSession' in navigator&&typeof MediaMetadata!=='undefined'){
    try{
      navigator.mediaSession.metadata=new MediaMetadata({title:'I’d Pick You First (remastered)',artist:'MusicSubject',album:'Call Me Daddy'});
