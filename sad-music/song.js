(()=>{
  ...
  document.getElementById('sadSongShare')?.addEventListener('click',async()=>{const lines=shareLines[song.id]||[`I found ${song.title}. Things escalated.`],text=lines[Math.floor(Math.random()*lines.length)],url=window.CMDShortLinks?.forTrack({songId:song.id,variantId:song.versions?.[0]?.id})||new URL(song.route,location.origin).href;try{if(navigator.share){await navigator.share({title:song.title,text:window.CMDShortLinks?.prepareShare({title:song.title,text,url}).text||`${text}\n${url}`});return}await navigator.clipboard.writeText(`${text}\n${url}`);const b=document.getElementById('sadSongShare');b.textContent='✓ Copied';setTimeout(()=>b.textContent='↗ Share',1400)}catch{}});
  const shareVersion=async version=>{const url=new URL('/music/',location.origin);url.searchParams.set('song',song.id);url.searchParams.set('version',version.id);url.searchParams.set('intent','heavy');url.searchParams.set('share','1');const data={title:`${song.title} — ${version.label}`,text:`Listen to ${song.title} — ${version.label}.`,url:url.href};if(window.CMDShare?.nativeShare)return window.CMDShare.nativeShare(data);try{if(navigator.share)return navigator.share({title:data.title,text:window.CMDShortLinks?.prepareShare(data).text||[data.text,data.url].filter(Boolean).join('\n')});await navigator.clipboard?.writeText(`${data.text}\n${data.url}`)}catch{}};
