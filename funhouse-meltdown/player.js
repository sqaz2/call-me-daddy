(()=>{
  const audio=document.getElementById('funhouseAudio'),wrap=audio?.closest('.audio-wrap'),label=wrap?.querySelector('.audio-label span'),title=wrap?.querySelector('.audio-label strong'),cover=document.querySelector('.machine-cover'),play=document.getElementById('funhousePlay');
  ...
  const controller=window.CMDContinuousPlayback.create({
    ...
    onPlayState:playing=>{status.textContent=playing?'Playing':(!audio.ended?'Paused':status.textContent);if(play)play.textContent=playing?'❚❚ Pause the song':'▶ Play the song'},
