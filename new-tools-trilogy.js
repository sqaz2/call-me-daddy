(()=>{
  ...
  function markSelected(key){document.querySelectorAll('[data-trilogy-track]').forEach(control=>{const selected=control.dataset.trilogyTrack===key;control.classList.toggle('is-selected',selected);control.setAttribute('aria-pressed',selected?'true':'false');const card=control.closest('.trilogy-card,.stone-player,.police-cover');if(card)card.classList.toggle('is-selected',selected)})}
  function updateDock(key){const track=tracks[key],i=order.indexOf(key);cover.src=track.cover;cover.alt=`${track.shortTitle} cover`;label.textContent=`Track ${i+1} of 3 · ${track.tone}`;title.textContent=track.title;title.href=track.page;progress.style.width='0%';showDock();markSelected(key)}
  ...
  function advance(){const i=order.indexOf(currentKey);if(i>=0&&i<order.length-1)selectTrack(order[i+1],true);else{wantsPlay=false;setPlaying(false,'Trilogy finished');progress.style.width='100%'}}
