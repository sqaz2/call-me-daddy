(()=>{
  ...
  const playCtas=[...document.querySelectorAll('[data-ss-play]')];
  ...
  const syncPlayCta=()=>{
    ...
    playCtas.forEach(button=>{
      button.textContent=localPlaying?'❚❚ Pause':'▶ Play the song';
      button.setAttribute('aria-label',localPlaying?'Pause the superstore effect':'Play the superstore effect');
    });
  };
  const media=track=>{
  ...
  const load=(target,autoplay=true)=>{
    ...
    if(autoplay)audio.play().catch(()=>{status.textContent='Ready · tap ▶ to continue';syncPlayCta()});
  };
  const ensureNext=()=>{
  ...
  const advance=()=>{
    ...
    if(target<0){status.textContent='Radio unavailable · open Music';play.textContent='▶';syncPlayCta();return;}
    load(target,true);
  };
  const start=()=>{
    if(index!==0)load(0,true);
    else if(audio.paused)audio.play().catch(()=>{status.textContent='Ready · tap ▶ to continue';syncPlayCta()});
  };

  playCtas.forEach(button=>button.addEventListener('click',()=>{
  ...
  audio.addEventListener('play',()=>{play.textContent='❚❚';status.textContent='Playing';syncPlayCta();window.CMDPersistentSite?.setSession(true);window.CMDPersistentSite?.refreshClearance?.();window.CMDCatalogCycle?.remember?.(current);try{navigator.mediaSession.playbackState='playing'}catch{}});
  audio.addEventListener('pause',()=>{if(!audio.ended){play.textContent='▶';status.textContent='Paused'}syncPlayCta();try{navigator.mediaSession.playbackState='paused'}catch{}});
  audio.addEventListener('ended',()=>{syncPlayCta();advance()});
  audio.addEventListener('timeupdate',()=>{if(audio.duration)bar.style.width=`${audio.currentTime/audio.duration*100}%`});
  audio.addEventListener('error',()=>{if(!audio.src)return;status.textContent='Skipping unavailable track…';syncPlayCta();window.setTimeout(advance,500)});
