(()=>{
  ...
  if(window.top!==window.self){
    ...
    const refresh=()=>{try{window.parent.postMessage({type:REFRESH},location.origin)}catch{}};
    const topSite=()=>{try{return window.top.location.origin===location.origin?window.top.CMDPersistentSite:null}catch{return null}};
    addEventListener('message',event=>{if(event.origin!==location.origin||event.source!==window.parent)return;if(event.data?.type===PAUSE)pauseLocal()});
    document.addEventListener('play',event=>{if(isAudibleMedia(event.target))claimPlayback()},true);
    window.CMDPersistentSite={
      ...
      makeSongLink(container,track,{show=false}={}){
        if(!container)return null;let link=container.querySelector('.cmd-now-song-link');
  ...
  const readPlaybackSnapshot=()=>{try{const s=JSON.parse(sessionStorage.getItem(PLAYBACK_KEY)||'null');return s&&s.wantsPlayback&&Date.now()-s.updatedAt<12*60*60*1000?s:null}catch{return null}};
  const removeResumePrompt=()=>{resumePrompt?.remove();resumePrompt=null};
  const offerPlaybackResume=()=>{
    const snapshot=readPlaybackSnapshot();if(!snapshot?.page||new URLSearchParams(location.search).get('cmdResume')==='1')return;
  ...
  const playerSelectors=[
    '.cmd-universal-player:not([hidden])','.ss-player:not([hidden])',
  ...
  const sameOriginUrl=value=>{try{const u=new URL(value,location.href);return u.origin===location.origin?u:null}catch{return null}};
  const armBackGuard=()=>{
    if(backGuardArmed||!initialReferrer)return;const target=sameOriginUrl(initialReferrer);if(!target)return;backGuardArmed=true;
    try{const current=history.state&&typeof history.state==='object'?history.state:{};history.replaceState({...current,cmdOwnerBase:true,cmdBackTarget:target.href},'',location.href);history.pushState({cmdOwnerGuard:true,cmdBackTarget:target.href},'',location.href)}catch{}
  };
  const playingMediaIn=win=>{try{return [...win.document.querySelectorAll('audio,video')].find(el=>isAudibleMedia(el)&&!el.paused&&!el.ended)||null}catch{return null}};
  ...
  const updatePillState=()=>{
    if(!overlay)return;const pill=overlay.querySelector('.cmd-site-session-pill');if(!pill)return;
  ...
  const claimOwner=win=>{
    if(!win)return;removeResumePrompt();
    const previousOwner=ownerWindow;ownerWindow=win;
    if(previousOwner!==win)pauseWindow(previousOwner);
    session=true;if(win===window)armBackGuard();cleanupFrames();updatePillState();scheduleClearance();
  };
  const visibleBottomClearance=(doc,view)=>{
    if(!doc||!view)return 0;const vh=view.innerHeight||document.documentElement.clientHeight||0;let clearance=0;const seen=new Set();
  ...
  const bindFrame=frame=>{
    try{
      ...
      doc.addEventListener('click',event=>{const a=event.target.closest('a[href]');if(!a||a.target||a.hasAttribute('download'))return;const url=sameOriginUrl(a.href);if(!url)return;const frameUrl=new URL(frame.contentWindow.location.href);if(url.pathname===frameUrl.pathname&&url.search===frameUrl.search&&url.hash)return;event.preventDefault();navigateInside(frame,url)},true);
    }catch{}
  };
  const makeFrame=url=>{
  ...
  const ensureOverlay=()=>{
    if(overlay)return;const style=document.createElement('style');
    style.textContent=`
  ...
  const openView=(url,{push=true}={})=>{
    ...
    if(viewFrame&&viewFrame.contentWindow!==ownerWindow)viewFrame.src=url.href;else makeFrame(url);updatePillState();scheduleClearance();
  };
  function openBackTarget(url){ensureOverlay();session=true;overlay.classList.add('is-open');document.documentElement.style.overflow='hidden';try{history.pushState({cmdView:url.href,cmdBackView:true},'',url.href)}catch{}makeFrame(url);updatePillState();scheduleClearance()}
  function returnToOwner(){
    if(ownerWindow===window){closeView();return}const ownerFrame=frameForWindow(ownerWindow);if(!ownerFrame)return;[...frames].forEach(frame=>{if(frame!==ownerFrame){frame.remove();frames.delete(frame)}});viewFrame=ownerFrame;ownerFrame.style.zIndex='1';
    try{history.pushState({cmdView:ownerWindow.location.href},'',ownerWindow.location.href)}catch{}
    updatePillState();watchFrameLayout();
  }
  function closeView({historyBack=false}={}){
    if(!overlay)return;if(ownerWindow!==window){returnToOwner();return}overlay.classList.remove('is-open');document.documentElement.style.overflow='';clearanceObserver?.disconnect();clearanceObserver=null;[...frames].forEach(f=>f.remove());frames.clear();viewFrame=null;
    if(!historyBack){internalNav=true;history.pushState({cmdHome:true},'',originUrl);internalNav=false}
  }
  function navigateInside(fromFrame,url){history.pushState({cmdView:url.href},'',url.href);if(fromFrame.contentWindow===ownerWindow)makeFrame(url);else{viewFrame=fromFrame;fromFrame.src=url.href}updatePillState()}
  document.addEventListener('play',event=>{if(isAudibleMedia(event.target))claimOwner(window)},true);
  document.addEventListener('click',event=>{
    if(!ownerIsActive())return;const a=event.target.closest('a[href]');if(!a||a.target||a.hasAttribute('download'))return;const url=sameOriginUrl(a.href);if(!url)return;const here=new URL(originUrl);if(url.pathname===here.pathname&&url.search===here.search&&url.hash)return;event.preventDefault();openView(url);
  },true);
  addEventListener('message',event=>{
    if(event.origin!==location.origin)return;const frame=[...frames].find(f=>f.contentWindow===event.source);if(!frame)return;
    if(event.data?.type===CLAIM)claimOwner(frame.contentWindow);else if(event.data?.type===REFRESH)scheduleClearance();
  });
  addEventListener('resize',scheduleClearance,{passive:true});addEventListener('orientationchange',scheduleClearance,{passive:true});
  addEventListener('popstate',event=>{
    const overlayOpen=Boolean(overlay?.classList.contains('is-open'));
    if(!overlayOpen){if(session&&ownerWindow===window&&event.state?.cmdOwnerBase){const url=sameOriginUrl(event.state.cmdBackTarget||initialReferrer);if(url){openBackTarget(url);return}}return}
