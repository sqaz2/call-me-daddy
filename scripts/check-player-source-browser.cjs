...
(async()=>{
 ...
 try{
  ...
  const state=()=>page.evaluate(()=>{const player=window.CMDUniversalPlayer,media=player?.getMedia();return {track:player?.getTrack(),source:media?.currentSrc,time:media?.currentTime,duration:media?.duration,paused:media?.paused,title:document.querySelector('.cmd-universal-title')?.textContent,mediaTitle:media?.ownerDocument.defaultView.navigator.mediaSession?.metadata?.title,artwork:document.querySelector('.cmd-universal-art img')?.src};});
