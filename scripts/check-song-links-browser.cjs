...
(async()=>{
 ...
 try{
  for(const active of [true,false]){
   ...
   await context.route(/https:\/\/(hiphop\.bid|dubstep\.bid|suno\.fyi|jokes\.win)\//,route=>active?route.fulfill({status:200,headers:{'access-control-allow-origin':'*','content-type':'application/json'},body:JSON.stringify({service:'musicsubject-song-links',revision:data.revision})}):route.abort());
   ...
   assert.equal(shared.url,active?`https://jokes.win/${joke.number}`:data.origin+'/funhouse-meltdown/');
   assert.equal(shared.activeGesture,true);assert.deepEqual(errors,[]);
   console.log(JSON.stringify({active,jokeShare:shared.url,gesture:shared.activeGesture,errors}));
   await context.close();
  }
