(()=>{
  'use strict';
  const ID='canadian-crop-dusting';
  ...
  const track={...song,...variant,id:`${ID}:main`,songId:ID,variantId:'main',variantCount:1,variantLabel:variant.label||'Original upload',title:song.title,cover:variant.cover||song.cover,radioIntent:'laugh'};
  ...
  const draw=()=>{
    ...
    buttons.forEach(button=>{button.setAttribute('aria-pressed',String(playing));button.setAttribute('aria-label',`${playing?'Pause':'Play'} Canadian Crop Dusting`);if(button.id!=='releasePlay')button.textContent=playing?'❚❚ Pause the song':'▶ Play the song'});
    const icon=document.getElementById('releasePlayIcon'),text=document.getElementById('releasePlayText');
    if(icon)icon.textContent=playing?'❚❚':'▶';
    if(text)text.textContent=`${playing?'Pause':'Play'} Canadian Crop Dusting`;
    if(status)status.textContent=message||(current.ours?`${playing?'Playing':'Paused'} · Canadian Crop Dusting · use the dock to seek or skip.`:current.playing?'Another song is playing in the dock. Tap here to return to Canadian Crop Dusting.':'Tap the artwork or Play. More music follows in the bottom player.');
  };
  const ensure=()=>{
    ...
    controller=window.CMDContinuousPlayback.create({id:ID,audio,tracks:[track],localCount:1,intent:'laugh',pageFollowSeconds:0,
