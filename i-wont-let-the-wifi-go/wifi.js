(()=>{
  ...
  const WAIT=4;
  const self={id:'i-wont-let-the-wifi-go',title:'I Won’t Let the Wi‑Fi Go',artist:'MusicSubject × Call Me Daddy',project:'2025 · early AI-music experiment',audio:'/media/songs/2025/i-wont-let-the-wifi-go/audio.mp3',cover:''};
  const catalog=(window.CMD_SONGS||[]).filter(s=>s?.audio&&s.id!==self.id).map(s=>({id:s.id,title:s.title,artist:s.artist||'Call Me Daddy',project:s.project||'Catalog',audio:s.audio,cover:s.cover||''}));
  ...
  const paint=track=>{status.textContent=track.title===self.title?'Play':track.title;nextUp.textContent=index===0?'Start here. The rest of the playable catalog follows automatically.':`Now playing · ${track.title}`;media(track)};
  ...
  audio.addEventListener('play',()=>{icon.textContent='❚❚';status.textContent=index===0?'Playing':queue[index].title;media(queue[index]);if('mediaSession'in navigator)navigator.mediaSession.playbackState='playing'});
  ...
  document.addEventListener('click',e=>{if(audio.paused||audio.ended)return;const a=e.target.closest('a[href]');if(!a||a.target||a.hasAttribute('download'))return;const u=new URL(a.href,location.href);if(u.origin!==location.origin)return;const same=u.pathname===location.pathname&&u.search===location.search;if(same&&u.hash)return;e.preventDefault();const opened=window.open(u.href,'_blank','noopener');if(!opened)location.href=u.href},true);
